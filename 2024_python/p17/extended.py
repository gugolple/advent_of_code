...
def entry_func(inp: str):
    ...
    cb = 0
    while True:
        cst = initialize_state(state)
        res = search_func(cst, program, idx)
        if res:
            st = cst
            break
        csob = cst[_STATE_OUT_B]
        if len(csob) > cb:
            print(idx, csob)
            cb = len(csob)
        idx += 1
    print(st)
    return ",".join([str(i) for i in st[_STATE_OUT_B]]), st, idx

if __name__ == "__main__":
    inp_str = sys.stdin.read()
    print(entry_func(inp_str[:-1])[2])
...
Program: 0,3,5,4,3,0"""
        out, state, val_a = entry_func(inp_str)
        self.assertEqual(out, "0,3,5,4,3,0")
        self.assertEqual(val_a, 117440)
