...
package body solutionpkp is
	...
	begin
		...
		while not End_Of_File (F) loop
			...
			multiplier := 1;
			if Contains(winners_map, current_id) then
				multiplier := winners_map(current_id);
			end if;
			Put_Line("Key: " & current_id'Image & " Multiplier: " & multiplier'Image & " LineT: " & line_total'Image);
			if line_total > 0 then
				-- Apply to all subsecuent
				for I in 1..line_total loop
					process_id := I + current_id;
					current_multiplier := 1;
					if winners_map.Contains(process_id) then
						current_multiplier := winners_map(process_id) + multiplier;
						winners_map(process_id) := current_multiplier;
						Put_Line("Set Key: " & process_id'Image & " to: " & current_multiplier'Image);
					else
						winners_map.insert(process_id, multiplier+1);
						Put_Line("Added Key: " & process_id'Image);
					end if;
				end loop;
			end if;
