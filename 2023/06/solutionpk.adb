...
package body solutionpk is
	package LongIntegerVector is new
		Ada.Containers.Vectors(
			Index_Type => Natural,
			Element_Type => Long_Integer
			);
	
	time_vector : LongIntegerVector.Vector;
	distance_vector: LongIntegerVector.Vector;
	state: StateMachine;
	...
	begin
		...
		case state is
			...
			when time_value =>
				...
				time_vector.append(current_number);
			when distance =>
				state := distance_value;
			when distance_value =>
				current_number := Long_Integer'Value(To_String(trimmed_string));
				distance_vector.append(current_number);
		end case;
	...
	begin
		...
		total := 0;
		for I in time_vector.first_index .. time_vector.last_index loop
			Put_Line("Idx: " & I'Image & " time: " & Long_Integer'image(time_vector(I)) & " distance: " & Long_Integer'image(distance_vector(I)));
			sub_total := 0;
			for J in 0 .. time_vector(I) loop
				my_distance := J * (time_vector(I) - J);
				if my_distance > distance_vector(I) then
