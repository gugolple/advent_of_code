...
package body solutionpkp is
	...
	begin
        ...
        procedure iteratePulse( nm : in out NameModMap.Map; total : in out
            Long_Integer; found : out Boolean) is
	    ...
	    pendig_queue.Enqueue(To_Holder(current_pulse));
            while pendig_queue.Current_Use > 0 loop
                ...
                if nm.Contains(current_pulse.destination) then
                    if current_pulse.destination = "rx" and current_pulse.hi_low = False then
                        found := True;
                    end if;
        ...
        function countPulses( nm : in out NameModMap.Map) return Long_Integer is
            ...
            count_press : Long_Integer := 1;
            found : Boolean := False;
        begin
            while found = False loop
                count_press := count_press + 1;
                iteratePulse(nm, total, found);
            end loop;
            return count_press;
        end countPulses;
