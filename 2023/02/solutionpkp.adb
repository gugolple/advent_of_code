...
package body solutionpkp is
	...
	begin
		...
		case state is
			...
			when Color =>
				...
				-- Keep track of minimun needed
				if current_number > MMin(To_String(trimmed_string)) then
					MMin(To_String(trimmed_string)) := current_number;
				end if;
				state := Number;
		end case;
	...
	begin
		...
		while not End_Of_File (F) loop
			...
			total := total + MMin("red") * MMin("blue") * MMin("green");
