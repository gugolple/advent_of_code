...
fn process_input(input: &str) -> u64 {
    ...
    for (pos_row, row) in grid.iter().enumerate() {
        for (col_num, col_chr) in row.iter().enumerate() {
            if *col_chr != 'a' && *col_chr != 'S' {
                continue;
            }
            println!("Row:{} Col:{}", pos_row, col_num);
            ...
            'outer: while let Some(cur_dij) = heap.pop() {
                if seen.contains(&cur_dij.pos) {
                    continue; 
                }
                if cur_dij.cost >= best {
                    break 'outer;
                }
